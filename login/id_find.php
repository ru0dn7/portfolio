        ...
        $(document).ready(function() {
            ...
            $(".find").click(function() {       // 아이디찾기 버튼을 클릭, id입력 상자에 id값 입력시
                ...
                $.ajax({    // ajax 로 data를 넘겨줌
                    type: "POST", // 전송방식
                    url: "find.php", // 접근할 파일 
                    data: "name="+ name+ "&hp1="+hp1+ "&hp2="+hp2+ "&hp3="+hp3, // 파라미터 매개변수id도 같이 넘겨줌 (이름, 전화번호1~3)
                    cache: false,  // 캐시 저장여부
    ...
    <article id="content">
        ...
        <div class="login_wrap">
            <form name="find" method="post" action="find.php">
                <ul>
                    <li>
                        <label class="hidden" for="name">아이디</label>
                    ...
                    <li class="hp">
                        ...
                        <input type="text" id="hp2" name="hp2" maxlength="4" required placeholder="(ex. 1111)">
                        <span>-</span>
                        <label class="hidden" for="hp3">연락처 마지막 4자리</label>
                        <input type="text" id="hp3" name="hp3" maxlength="4" required placeholder="(ex. 1111)">
                    </li>
                </ul>
